...
local CFG = {
	...
	On = { Items = true, Cursed = true, Ghost = true, Temp = true, Bright = true, Dead = true,
		Hunt = true, Exit = true, Hide = true, Mate = true, Room = true,
...
local CURSED_WORDS = "通灵板 显灵板 占卜板 幽灵板 魔板 诅咒镜 魔镜 邪镜子 血镜 骨镜 放大镜 音乐盒 八音盒 召唤阵 召唤圈 预言家 算命机 占卜机 塔罗牌 巫毒娃娃 骷髅头 诅咒娃娃 诅咒之书"
local CURSED = {}
for w in string.gmatch(CURSED_WORDS, "%S+") do CURSED[w] = w end
CURSED["Umbra板"] = "通灵板"
local CURSED_EN = "UmbraBoard 通灵板 Umbra 通灵板 Umbraplate 通灵板 OuijaBoard 通灵板 Ouija 通灵板 SpiritBoard 通灵板 TalkingBoard 通灵板 Planchette 通灵板 HauntedMirror 诅咒镜 CursedMirror 诅咒镜 HauntedMirror2 诅咒镜 MagnifyingGlass 放大镜 Magnifier 放大镜 MusicBox 音乐盒 MusicBoxes 音乐盒 SummoningCircle 召唤阵 Summoning 召唤阵 FortuneTeller 预言家 FortuneTellerMachine 预言家 TarotCards 塔罗牌 Tarot 塔罗牌 VoodooDoll 巫毒娃娃 Skull 骷髅头 CursedDoll 诅咒娃娃 CursedBook 诅咒之书 BloodMirror 血镜 BoneMirror 骨镜"
...
local BOX_NAMES = {
	spiritbox = true, spiritboxes = true, box = true, ghostbox = true,
...
local function CheckTemp()
	...
	if cur then
		...
		if S.TempText then
			pcall(function()
				local src = S.TempSrc and ("[" .. S.TempSrc .. "]") or ""
...
local function RescanItems()
	...
	S.Cursed = {}
	local n = 0
	local containers = { "Items", "CursedPossessions", "CursedPossessionHolder",
		"CursedPossession", "Cursed", "CursedItems", "CursedObjects",
		"Props", "Pickups", "Interactables", "Special", "Rare" }
	local map = WS:FindFirstChild("Map")
	if map then
		for _, cn in ipairs(containers) do
			local f = map:FindFirstChild(cn)
			if f then containers[#containers + 1] = f end
		end
	end
	local scanned = {}
	for _, cn in ipairs(containers) do
		local f = type(cn) == "string" and WS:FindFirstChild(cn) or cn
		if f and not scanned[f] then
			scanned[f] = true
			pcall(function()
	...
	for obj in pairs(S.Mark) do
		if not keep[obj] then
			local isGhost = false
			if g then
				isGhost = (obj == g) or (obj.Parent ~= nil and obj:IsDescendantOf(g))
			end
			if not isGhost then
				Unmark(obj)
			end
...
local function ScanBoxReply()
	local got = false
	local hasTool = (FindTool(BOX_NAMES) ~= nil)
	...
	pcall(function()
		local tool = FindTool(BOX_NAMES)
		if not tool then return end
		for k, val in pairs(tool:GetAttributes()) do
			local kk = Norm(k)
			if string.find(kk, "respond", 1, true) or string.find(kk, "reply", 1, true)
				or string.find(kk, "answer", 1, true) or string.find(kk, "spirit", 1, true)
				or string.find(kk, "active", 1, true) or string.find(kk, "used", 1, true) then
				if val == true or val == 1 or tostring(val) == "true" then got = true return end
			end
		end
	end)
	if got then return "BOX" end
...
local function ScanEvidenceObjects()
	...
	local function put(e)
		if e and not seen[e] then seen[e] = true found[#found + 1] = e end
	end
	...
	local function scanFolder(f)
		...
		pcall(function()
			for _, d in ipairs(f:GetChildren()) do
				if not isPlayerStuff(d) then
					local ev = MatchEvGroup(d.Name)
					if ev then
						if d:IsA("Folder") then
							if #d:GetChildren() > 0 then put(ev) end
						else
							put(ev)
						end
					end
		...
		if o then
			...
			if ev then
				if o:IsA("Folder") then
					if #o:GetChildren() > 0 then put(ev) end
				else
					put(ev)
				end
			end
	...
	pcall(function()
			...
			if not isPlayerStuff(v) then
				...
				if ev then
					if v:IsA("Folder") then
						if #v:GetChildren() > 0 then put(ev) end
					else
						put(ev)
					end
				end
		...
		for k, v in pairs(g:GetAttributes()) do
			...
			if ev and (v == true or v == 1 or tostring(v) == "true") then put(ev) end
		end
	end)
	scanFolder(WS)
	pcall(function()
		local m = WS:FindFirstChild("Map")
		if m then scanFolder(m) end
	end)
	pcall(function()
		local cs = GetRooms()
		if cs then for _, c in ipairs(cs) do scanFolder(c) end end
	end)
	if S.LowestTemp and S.LowestTemp < CFG.FreezePoint then put("FREEZE") end
	put(ScanEMF())
	put(ScanBoxReply())
	put(ScanBookWriting())
	pcall(function()
		for _, ev in ipairs(ScanJournal()) do put(ev) end
	end)
	local out = {}
	for _, ev in ipairs(found) do out[#out + 1] = EvCN(ev) end
	table.sort(out)
	return out, found
end
...
local function CheckEvidence()
	local out, keys = ScanEvidenceObjects()
...
local function DetectHunt()
	...
	pcall(function()
		local g = GetGhost()
		local b = ReadAttrBool(g, HUNT_KEYS)
		if b == true then hunting = true end
		if not hunting then
			for _, o in ipairs(WS:GetChildren()) do
				if IsNameHit(o.Name, HUNT_KEYS) then
					local bb = ReadAttrBool(o, HUNT_KEYS)
					if bb == true then hunting = true break end
				end
			end
		end
		if not hunting then
			local e = GetEnergy()
			if e and e < CFG.HuntThreshold then hunting = true end
		end
	end)
	return hunting
end
...
local function HuntAlert(on)
	if on and not S.HuntNotified then
		S.HuntNotified = true
		Notify("猎杀预警", "鬼开始猎杀了 快躲起来或跑", 6, "hunt")
		pcall(function()
	...
	pcall(function()
		...
		for _, p in ipairs(plrs:GetPlayers()) do
			if p ~= LP and p.Character then
				Mark(p.Character, CFG.MateColor)
				count = count + 1
			end
		end
	end)
...
local function BuildUI()
	...
	p.Size = UDim2.new(0.48, 0, 0.92, 0)
	p.Position = UDim2.new(0.50, 0, 0.03, 0)
	...
	for i = 1, 44 do
		pcall(function()
			...
			l.Position = UDim2.new(0, 5, 0, 46 + (i - 1) * 13)
...
local function EnsureUI()
	...
	if n < 20 or not S.Gui or not S.Gui.Parent then
		pcall(BuildUI)
	end
end
...
local function DrawUI()
	...
	if S.Hunting then
		...
		local d = S.HuntDist
		if d then add("  鬼距离 " .. Fmt1(d) .. "m", CFG.HuntColor) end
	...
	if S.GhostDist and not S.Hunting then
		add("鬼距离 " .. Fmt1(S.GhostDist) .. "m", Color3.fromRGB(255, 200, 200))
	end
	...
	if S.Hunting or en or S.GhostDist or (S.FuseOn ~= nil) or (CFG.On.FPS and S.FPS) or (S.MateCount and S.MateCount > 0) then
		y = y + 1
	end
	...
	if #S.Evidence == 0 then
		...
		for _, e in ipairs(S.Evidence) do add("  " .. e, Color3.fromRGB(230, 255, 150)) end
	end
	y = y + 1
