...
local function SolveGhost(keys)
	...
	if hasOrb then
		...
		if #h2 == 1 then
			S.FakeOrb = true
			S.FakeOrbWhy = S.FakeOrbWhy or "去掉灵球后唯一匹配"
			return h2[1], { h2[1] }
		end
		if #rest > 0 then
			local h3 = containing(rest)
			local hAll = containing(keys)
			-- 加上灵球后反而没有任何鬼能匹配 => 灵球是假的
			if #hAll == 0 and #h3 >= 1 then
				S.FakeOrb = true
				S.FakeOrbWhy = S.FakeOrbWhy or "加灵球后无鬼匹配"
				return nil, h3
			end
			if #h3 >= 1 then
				if #hAll == 0 then S.FakeOrb = true end
				return nil, h3
			end
		end
	end
...
local function CheckEvidence()
	...
	-- 灵球闪烁追踪: 忽有忽无 多为 Skinwalker/Doppelganger 伪造
	local orbNow = false
...
-- 官方死亡检测: Humanoid.Died 事件
-- Health=0 / State=Dead / RequiresNeck断裂 都会触发, 比轮询可靠
...
local function DrawUI()
	...
	if #S.Evidence == 0 then
		...
		for _, e in ipairs(S.Evidence) do
			...
			local fk = ""
			if EvCN("ORB") == e and (S.FakeOrb or S.OrbFlicker) then
				fk = " (疑假)"
			end
...
API.OrbFake = function()
	if S.FakeOrb then return true, (S.FakeOrbWhy or "证据组合判定") end
	if S.OrbFlicker then return true, "灵球反复出现消失" end
	return false, nil
end

API.OrbInfo = function()
	return {
		fake = (S.FakeOrb == true) or (S.OrbFlicker == true),
		why = S.FakeOrbWhy,
