...
local EV_TOOL = {
	flowerpot = 1, flower = 1, flowers = 1, pot = 1, plant = 1, vase = 1,
	spiritbook = 1, spiritbooks = 1, book = 1, books = 1, notebook = 1,
	journal = 1, diary = 1, logbook = 1, textbook = 1,
	projector = 1, laserprojector = 1, laserproj = 1, laser = 1,
	laserpointer = 1, beam = 1, discoball = 1,
	["花盆"] = 1, ["花"] = 1, ["花朵"] = 1, ["盆栽"] = 1, ["植物"] = 1, ["花瓶"] = 1,
	["灵书"] = 1, ["灵魂之书"] = 1, ["书"] = 1, ["书籍"] = 1, ["笔记"] = 1,
	["笔记本"] = 1, ["日志"] = 1, ["日记"] = 1, ["课本"] = 1,
	["投影仪"] = 1, ["投影"] = 1, ["激光"] = 1, ["激光灯"] = 1, ["镭射"] = 1,
}
local function IsEvTool(n)
	local k = Norm(n)
	if EV_TOOL[k] then return true end
	for kw in pairs(EV_TOOL) do
...
local EV_OBJ = {
	HAND = { handprint = 1, handprints = 1, fingerprint = 1, fingerprints = 1,
		footprint = 1, footprints = 1, print = 1, prints = 1, saltprint = 1,
		["手印"] = 1, ["指纹"] = 1, ["脚印"] = 1, ["掌印"] = 1, ["印痕"] = 1 },
	ORB = { orb = 1, orbs = 1, ghostorb = 1, ghostorbs = 1,
		ghostorbfolder = 1, orbparticle = 1,
		["鬼球"] = 1, ["灵球"] = 1, ["光球"] = 1, ["幽灵球"] = 1 },
	WRITE = { writing = 1, ghostwriting = 1, ghostwritings = 1,
		inscription = 1, inscriptions = 1, bookwriting = 1, scribble = 1,
		["鬼写字"] = 1, ["鬼写书"] = 1, ["铭文"] = 1, ["题字"] = 1,
		["字迹"] = 1, ["涂鸦"] = 1, ["刻痕"] = 1, ["留言"] = 1 },
	WITHER = { withered = 1, wilted = 1, wilt = 1, rusted = 1, decayed = 1,
		decay = 1, distorted = 1, deadflower = 1, deadplant = 1, rotten = 1,
		moldy = 1, corrupted = 1,
		["枯萎"] = 1, ["凋谢"] = 1, ["干枯"] = 1, ["生锈"] = 1, ["锈蚀"] = 1,
		["腐蚀"] = 1, ["扭曲"] = 1, ["腐烂"] = 1, ["腐坏"] = 1, ["发霉"] = 1 },
	LASER = { laserevidence = 1, lasersilhouette = 1, ghostinlaser = 1,
		lasermanifest = 1, laserfigure = 1, projectionevidence = 1,
		["激光显形"] = 1, ["投影显形"] = 1, ["光束显形"] = 1 },
}
local WITHER_STATE = { wilted = 1, withered = 1, dead = 1, decayed = 1,
	rotten = 1, dried = 1, brown = 1, deadflower = 1, deadplant = 1,
	["枯萎"] = 1, ["凋谢"] = 1, ["干枯"] = 1, ["枯死"] = 1, ["死亡"] = 1 }
local function ScanWither()
	local hit = nil
	local budget = 600
	local function walk(f, depth)
		if hit or budget <= 0 or depth > 3 then return end
		for _, d in ipairs(f:GetChildren()) do
			...
			local k = Norm(nm)
			if EV_OBJ.WITHER[k] then hit = d return end
			for kw in pairs(EV_OBJ.WITHER) do
				if string.find(k, Norm(kw), 1, true) then hit = d return end
			end
			local c = 0
	...
	pcall(function() walk(WS, 1) end)
	if hit then return "WITHER", "场景枯萎" end
	pcall(function()
		local pots = {}
		local function findPot(f, depth)
			if #pots >= 12 or budget <= 0 or depth > 3 then return end
			for _, d in ipairs(f:GetChildren()) do
				budget = budget - 1
				if budget <= 0 then return end
				local k = Norm(tostring(d.Name))
				if k == "flowerpot" or k == "花盆" or k == "flower" or k == "花"
					or string.find(k, "flowerpot", 1, true) then
					pots[#pots + 1] = d
				else
					local c = 0
					pcall(function() c = #d:GetChildren() end)
					if c > 0 and c < 200 then findPot(d, depth + 1) end
				end
			end
		end
		pcall(function() findPot(WS, 1) end)
		for _, pot in ipairs(pots) do
			local ok = false
			pcall(function()
				for k, v in pairs(pot:GetAttributes()) do
					local kk = Norm(k)
					if WITHER_STATE[kk] and (v == true or v == 1 or tostring(v) == "true") then
						ok = true
					end
				end
			end)
			if ok then hit = pot return end
			pcall(function()
				for _, ch in ipairs(pot:GetDescendants()) do
					local kk = Norm(tostring(ch.Name))
					if WITHER_STATE[kk] then hit = pot return end
					local isDead = false
					pcall(function()
						for k, v in pairs(ch:GetAttributes()) do
							if WITHER_STATE[Norm(k)] and (v == true or v == 1 or tostring(v) == "true") then
								isDead = true
							end
						end
					end)
					if isDead then hit = pot return end
				end
			end)
			if hit then return end
		end
	end)
	if hit then return "WITHER", "花盆枯萎" end
	return nil, nil
end
local GHOST_DB = {
...
local function MatchEvGroup(n)
	...
	if IsEvTool(n) then return nil end
	return nil
end
...
local function ScanEvidenceObjects()
	...
	if S.ManEv then
		local cnt = 0
		for _ in pairs(S.ManEv) do cnt = cnt + 1 end
		if cnt > 0 then
			local out2 = {}
			for _, k in ipairs({ "EMF", "HAND", "BOX", "FREEZE", "ORB", "WRITE",
				"LASER", "WITHER" }) do
				if S.ManEv[k] then put(k, "手动") end
			end
			for _, ev in ipairs(found) do out2[#out2 + 1] = EvCN(ev) end
			table.sort(out2)
			return out2, found, src
		end
	end
	...
	local wt, wtsrc = ScanWither()
	if wt then put(wt, wtsrc or "枯萎") end
	pcall(function()
...
local function SolveGhost(keys)
	...
	if hasOrb then
		...
		if #h2 == 1 then
			S.FakeOrb = true
			S.FakeOrbWhy = "去掉灵球后唯一匹配 " .. (GHOST_CN[h2[1]] or h2[1])
			return h2[1], { h2[1] }
		end
		if #keys >= 4 then
			S.OrbSuspect = true
			S.OrbSuspectWhy = "证据" .. #keys .. "条(应为3条), 可能有误报"
		end
		if #rest > 0 then
			local h3 = containing(rest)
			local hAll = containing(keys)
			if #hAll == 0 and #h3 >= 1 then
				S.OrbSuspect = true
				S.OrbSuspectWhy = S.OrbSuspectWhy or "含灵球无匹配, 去掉后有" .. #h3 .. "个候选"
			end
...
local function CheckEvidence()
	...
	local orbNow = false
	for _, k in ipairs(keys) do if k == "ORB" then orbNow = true end end
	if S.OrbLast == nil then
		S.OrbLast = orbNow
	elseif S.OrbLast ~= orbNow then
		S.OrbLast = orbNow
		S.OrbFlips = (S.OrbFlips or 0) + 1
	end
	S.OrbFlicker = (S.OrbFlips or 0) >= 4
	S.EvSrc = src or {}
...
BindDeathEvents = function()
	local h = GetHum()
	if not h then return end
	if S.DiedBound == h then return end
	S.DiedBound = h
	...
	pcall(function()
		if h.Died then
			S.Conn[#S.Conn + 1] = h.Died:Connect(onDead)
		end
	end)
	pcall(function()
		if h.HealthChanged then
			S.Conn[#S.Conn + 1] = h.HealthChanged:Connect(function(hp)
				if hp and hp <= 0 then onDead() end
			end)
		end
	end)
	pcall(function()
		if h.StateChanged then
			S.Conn[#S.Conn + 1] = h.StateChanged:Connect(function(_, ns)
				if ns and tostring(ns):find("Dead") then onDead() end
			end)
		end
	end)
end
	...
	if #S.Evidence == 0 then
		...
		for _, e in ipairs(S.Evidence) do
			...
			if EvCN("ORB") == e then
				if S.FakeOrb then
					fk = " (假)"
				elseif S.OrbSuspect then
					fk = " (存疑)"
				elseif S.OrbFlicker then
					fk = " (闪烁)"
				end
			end
			add("  " .. e .. fk .. (sname and (" (" .. sname .. ")") or ""), Color3.fromRGB(230, 255, 150))
		end
	end
	y = y + 1
...
API.OrbFake = function()
	if S.FakeOrb then return true, (S.FakeOrbWhy or "去掉灵球后唯一匹配") end
	return false, nil
end
API.OrbInfo = function()
	return {
		fake = (S.FakeOrb == true),
		suspect = (S.OrbSuspect == true),
		why = S.FakeOrbWhy or S.OrbSuspectWhy,
		flicker = S.OrbFlicker == true,
		flips = S.OrbFlips or 0,
	}
end
...
API.SetEvidence = function(list)
	S.ManEv = {}
	if not list then S.ManEvLock = nil pcall(Tick) return 0 end
	local n = 0
	for w in string.gmatch(tostring(list), "[^,%s/、]+") do
		local k = EvKeyFromCN(w)
		if k and not S.ManEv[k] then S.ManEv[k] = true n = n + 1 end
	end
	S.ManEvLock = (n > 0)
	pcall(Tick)
	return n
